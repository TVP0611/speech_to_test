...
if __name__ == '__main__':
    wav_loc = 'remove_noise_in_audio.wav'
    src_rate, audio_clip_cafe = wavfile.read(wav_loc)
    audio_clip_cafe = audio_clip_cafe.astype(np.float32)
    # src_data = np.concatenate((src_data, np.zeros(src_rate*3)))
    # src_data = audio_clip_cafe / 32768
    wav_loc = 'noise.wav'
    noise_rate, noise_clip = wavfile.read(wav_loc)
    ...
    sf.write("{0}.wav".format("remove_noise_in_audio"), output, 8000, 'PCM_16')
