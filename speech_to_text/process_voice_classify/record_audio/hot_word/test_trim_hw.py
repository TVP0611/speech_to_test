...
def sd_callback(rec, frames, time, status):
    ...
    rec = np.squeeze(rec)
    # print(len(rec))
    # sf.write('acis/' + "{0}_{1}.wav".format("rec_before", file_save), rec, 48000, 'PCM_16')

    # Resample
    rec, new_fs = decimate(rec, sample_rate, resample_rate)
    # print(len(rec))
    print(new_fs)
    sf.write('acis/' + "{0}_{1}.wav".format("rec", file_save), rec, 8000, 'PCM_16')
    # print(len(window))
    # Save recording onto sliding window
    window[:len(window) // 2] = window[len(window) // 2:]
    window[len(window) // 2:] = rec
    # print(len(window))
    # sf.write('acis/' + "{0}_{1}.wav".format("win", file_save), rec, 8000, 'PCM_16')
    # Compute features
